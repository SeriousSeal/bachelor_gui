...
const NODE_TYPES = {
  custom: React.memo(({ data }) => {
    ...
    const displayData = useMemo(() => {
      ...
      const operationPercentage = data.showOperations && data.operations && data.totalOperations
        ? `${(data.operations * 100 / data.totalOperations).toLocaleString()}%`
        : null;

      return { displayLabel, nodeWidth, operationPercentage };
    }, [data.label, data.showOperations, data.operations, data.totalOperations]);
...
const Flow = ({   
  ...
  handleOptionClick = () => {} // Default no-op function }) => {
    ...
    const augmentedNodes = useMemo(() => {
      ...
      const operationsData = showOperations ? {
        showOperations: true,
